# Домашнее задание 5.
# Задача 2. про конфеты - заменено на АПЕЛЬСИНы
# Создайте программу для игры с АПЕЛЬСИНами человек против человека.
# Условие задачи: На столе лежит 2021 АПЕЛЬСИНа. Играют два игрока делая ход друг после друга. 
# Первый ход определяется жеребьёвкой. 
# За один ход можно забрать не более чем 28 АПЕЛЬСИНов. 
# Все АПЕЛЬСИНы оппонента достаются сделавшему последний ход. 
# Сколько АПЕЛЬСИНов нужно взять первому игроку, чтобы забрать все АПЕЛЬСИНы у своего конкурента?
...
def mode():
    ...
    if not mode_.isdigit() or int(mode_) > 1 or int(mode_) < 0:
        ...
        print("\033[Неккоректный ввод!!!\033[\n")
        mode_ = mode()
    return int(mode_)
